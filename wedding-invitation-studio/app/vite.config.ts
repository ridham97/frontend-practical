  ...
  return {
    resolve: {
      alias: [
        ...
        // Dev-only: workerd's `cloudflare:workers` does not exist in local
        // Node, so point it at an empty-env stub (deploys keep it external).
        ...(command === "serve"
          ? [
              {
                find: /^cloudflare:workers$/,
                replacement: fileURLToPath(new URL("./src/lib/cloudflare-workers.dev-stub.ts", import.meta.url)),
              },
            ]
          : []),
      ],
    ...
    // Dev-only: keep the workerd built-in out of the dependency scan so CJS
    // pre-bundling still runs (a failed scan breaks react interop in dev SSR).
    optimizeDeps: {
      exclude: ["cloudflare:workers"],
    },
    ssr: {
      // Bundle everything for the Worker build; in dev, let Node resolve npm
      // deps natively (the ESM module runner cannot execute CJS like react).
